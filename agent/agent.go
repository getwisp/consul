...
// WithFlags specifies the command line flags that the agent
...
func New(options ...AgentOption) (*Agent, error) {
	...
	config, warnings, err := autoconf.LoadConfig(flat.flags, config.Source{}, flat.overrides...)
	if err != nil {
		return nil, err
	}
