...
func TestLoadConfig(t *testing.T) {
	...
	cfg, warnings, err := LoadConfig(flags, config.Source{
		Name:   "test",
		Format: "hcl",
...
func TestReadConfig(t *testing.T) {
	...
	devMode := true
	ac := AutoConfig{
		autoConfigData: `{"NodeName": "hobbiton"}`,
		flags: config.Flags{
			// putting this in dev mode so that the config validates
			// without having to specify a data directory
			DevMode: &devMode,
...
func testSetupAutoConf(t *testing.T) (string, string, config.Flags) {
	t.Helper()
	...
	return dataDir, configDir, flags
}

func TestInitialConfiguration_disabled(t *testing.T) {
	dataDir, configDir, flags := testSetupAutoConf(t)
	...
	flags.ConfigFiles = append(flags.ConfigFiles, cfgFile)

	directRPC := mockDirectRPC{}
	ac, err := New(WithFlags(flags), WithTLSConfigurator(&tlsutil.Configurator{}), WithDirectRPC(&directRPC))
	require.NoError(t, err)
	require.NotNil(t, ac)
...
func TestInitialConfiguration_cancelled(t *testing.T) {
	_, configDir, flags := testSetupAutoConf(t)
...
func TestInitialConfiguration_retries(t *testing.T) {
	...
	ac, err := New(WithFlags(flags), WithTLSConfigurator(&tlsutil.Configurator{}), WithDirectRPC(&directRPC), WithRetryWaiter(waiter))
	require.NoError(t, err)
	require.NotNil(t, ac)
