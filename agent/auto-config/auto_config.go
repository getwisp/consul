...
// WithFlags will cause the created AutoConfig type to use the provided CLI flags
...
func LoadConfig(flags config.Flags, extraHead config.Source, overrides ...config.Source) (*config.RuntimeConfig, []string, error) {
...
func (ac *AutoConfig) ReadConfig() (*config.RuntimeConfig, error) {
	...
	cfg, warnings, err := LoadConfig(ac.flags, src, ac.overrides...)
	if err != nil {
		return cfg, err
	}
